...
export function NotificationCenter() {
  ...
  const markAsRead = async (id: string) => {
    ...
    // Left unread on failure, so the badge does not hide one that is still unread.
    if (error) return;
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button aria-label={unreadCount > 0 ? `Notifications, ${unreadCount} unread` : "Notifications"} variant="ghost" size="icon" className="relative h-9 w-9">
