  ...
  const dialogContent = (
    <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
      ...
      <div className="space-y-4">
        ...
        {people.length > 0 && <div className="space-y-2"><Label className="text-xs">People</Label><div className="flex flex-wrap gap-x-4 gap-y-1.5">{people.map((p) => <label key={p.id} className="flex items-center gap-1.5 text-sm cursor-pointer"><Checkbox checked={matchedPeople.includes(p.id)} onCheckedChange={(checked) => setMatchedPeople((prev) => checked ? [...prev, p.id] : prev.filter((id) => id !== p.id))} />{p.name}</label>)}</div></div>}
