...
export function ContactTopicHistory({ topicId }: { topicId: string }) {
  ...
  if (query.isError) return <p role="alert" className="text-sm text-destructive">History could not be loaded.</p>;
  const reversed = new Set(query.data.map(event => event.reverses_event_id).filter(Boolean));
  return <ol aria-label="Topic history" className="space-y-2 border-l-2 border-border pl-3 text-xs text-muted-foreground">
