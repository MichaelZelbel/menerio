...
export function ContactTopicRow({ topic, pending, onCommand }: { topic: ContactTopic; pending: boolean; onCommand: (command: TopicCommand) => Promise<boolean> }) {
  ...
  return <li className="min-w-0 space-y-3 py-4" aria-label={topic.title}>
    {editing ? <form className="space-y-3" onSubmit={async e => {
      e.preventDefault();
      if (await onCommand({ action: 'update', ...base, patch: { title: title.trim(), priority, mode } })) setEditing(false);
    }}>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button type="button" variant="ghost" className="min-h-11" disabled={pending} onClick={() => setEditing(false)}>Cancel</Button>
      </div>
    </form> : <>
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        <Button variant="ghost" className="min-h-11 gap-1.5" disabled={pending} onClick={() => { setTitle(topic.title); setPriority(topic.priority); setMode(topic.mode); setEditing(true); }}><Pencil aria-hidden="true" className="h-3.5 w-3.5" />Edit</Button>
