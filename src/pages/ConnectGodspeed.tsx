...
import { answerConnectRequest, getConnectRequest, type GodspeedConnectRequest } from "@/lib/mc-connect";

/**
 * The page a mission control sends its owner to: "this mission control wants to connect, is that you?"
 *
 * Mission Control started the request and is waiting. This page shows who is asking and
 * for what, the person compares the code with the one their mission control shows, and says
 * yes or no. No key is shown here or anywhere: Mission Control collects it by itself
...
type View =
  | { kind: "loading" }
  | { kind: "gone" }
  | { kind: "confirm"; request: GodspeedConnectRequest }
  | { kind: "approved"; godspeedName: string }
  | { kind: "denied"; tooManyTries: boolean }
  | { kind: "error"; message: string };

export default function ConnectGodspeed() {
  ...
  // wrong, the person types what their mission control shows instead.
  ...
  const answer = async (approve: boolean) => {
    ...
    if (res.ok && res.data) {
      setView(res.data.status === "approved"
        ? { kind: "approved", godspeedName: view.request.godspeed_name }
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4 py-12">
      ...
      <div className="w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-2xl font-bold font-display">Connect your mission control</h1>
        </div>
        ...
        {view.kind === "gone" && (
          <Message
            ...
            body={
              requestId
                ? "It may have expired, been answered already, or been opened with a different account. Requests last ten minutes. Start again from your mission control."
                : `This page opens from your mission control. Tell your mission control assistant: connect ${BRAND.name}.`
            }
          />
        )}
        ...
        {view.kind === "approved" && (
          <Message
            ...
            body={`${view.godspeedName} finishes the rest by itself. You can end the connection any time under Settings, Integrations.`}
          />
        )}
        ...
        {view.kind === "denied" && (
          <Message
            ...
            body={
              view.tooManyTries
                ? "This request was closed and nothing was connected. Start again from your mission control."
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">
                {view.request.reconnect ? "Connect this mission control again?" : "Connect this mission control?"}
              </CardTitle>
              <CardDescription>
                {view.request.reconnect
                  ? "This mission control has been connected to your account before. Connecting again gives it fresh access, and any earlier access stops working."
                  : `A mission control is asking for access to your ${BRAND.name} account.`}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
              <dl className="space-y-2 text-sm">
                <Row label="Account" value={view.request.account_label} />
                <Row label="Godspeed" value={view.request.godspeed_name} />
                <Row label="Computer" value={view.request.device_name} />
              </dl>

              {!askForCode ? (
                <div className="rounded-lg border bg-muted/40 p-4 text-center">
                  <p className="text-xs text-muted-foreground">Check that your mission control shows the same code</p>
                ...
                <div className="space-y-2">
                  <Label htmlFor="mc-code">Type the code your mission control shows</Label>
                  <Input
                    id="mc-code"
                    value={typedCode}
              ...
              <ul className="space-y-2 text-sm">
                <li className="flex gap-2">
                  ...
                  <span>
                    Your mission control's assistants can read your profile, people, notes and facts in {BRAND.name}, and save notes for you.
                  </span>
                </li>
                <li className="flex gap-2 text-muted-foreground">
                  <Minus className="mt-0.5 h-4 w-4 shrink-0" />
                  <span>No files from your mission control are copied.</span>
                </li>
              </ul>
            </CardContent>
