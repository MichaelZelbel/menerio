...
const open = {
  ...
  data: {
    hub_name: "Home hub", device_name: "Laptop", flow: "browser", wants: { context: true, documents: false },
...
const renderAt = (search: string) =>
  render(<MemoryRouter initialEntries={[`/connect-hub${search}`]}><ConnectHub /></MemoryRouter>);
...
describe("ConnectHub", () => {
  it("shows the account, the hub, the computer, the code to compare, and what connecting allows", async () => {
    getRequest.mockResolvedValue(open);
    renderAt(`?request=${REQUEST_ID}&code=BCDF-GHJK`);
    expect(await screen.findByText("Connect this hub?")).toBeInTheDocument();
    ...
    expect(screen.getByText(`Your hub's assistants can read your profile, people, notes and facts in ${BRAND.name}, and save notes for you.`)).toBeInTheDocument();
    expect(screen.getByText("No files from your hub are copied.")).toBeInTheDocument();
  });
  ...
  it("asks for the code when the link has none, and again when it was wrong", async () => {
    ...
    const input = await screen.findByLabelText("Type the code your hub shows");
  ...
  it("tells a returning hub that earlier access stops working", async () => {
    getRequest.mockResolvedValue({ ...open, data: { ...open.data, reconnect: true } });
    renderAt(`?request=${REQUEST_ID}&code=BCDF-GHJK`);
    expect(await screen.findByText("Connect this hub again?")).toBeInTheDocument();
  });
  ...
  it("says the request is not open for a 404, and does not ask the server without a request", async () => {
    ...
    expect(screen.getByText(/Start again from your hub/)).toBeInTheDocument();
    first.unmount();

    getRequest.mockClear();
    renderAt("");
    expect(await screen.findByText(/This page opens from your hub/)).toBeInTheDocument();
