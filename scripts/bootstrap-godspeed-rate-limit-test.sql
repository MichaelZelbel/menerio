-- Disposable fixture only. Run against a fresh godspeed_rate_limit_test database.
DO $$BEGIN IF current_database()<>'godspeed_rate_limit_test' THEN RAISE EXCEPTION 'Wrong database'; END IF; END$$;
DO $$BEGIN
...
CREATE TABLE godspeed_api_keys(id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid NOT NULL);
CREATE TABLE godspeed_api_usage(
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  key_id uuid NOT NULL REFERENCES godspeed_api_keys(id),
  window_start timestamptz NOT NULL,
  request_count integer NOT NULL DEFAULT 1,
  created_at timestamptz DEFAULT now(),
  CONSTRAINT godspeed_api_usage_key_window_unique UNIQUE (key_id, window_start)
);
...
BEGIN
  SELECT u.request_count INTO v_count FROM godspeed_api_usage u
  ...
  INSERT INTO godspeed_api_usage (key_id, window_start, request_count)
