-- Disposable fixture only. Run against a fresh godspeed_connect_test database, then
-- run scripts/test-godspeed-connect.mjs, which applies the real migration on top.
DO $$BEGIN IF current_database()<>'godspeed_connect_test' THEN RAISE EXCEPTION 'Wrong database'; END IF; END$$;
DO $$BEGIN
...
-- godspeed_api_keys as production has it before this work: the table of
...
CREATE POLICY "Users can manage own API keys" ON public.godspeed_api_keys
