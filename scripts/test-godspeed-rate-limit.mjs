/**
 * Mission Control API throttle must hold when requests arrive together, not only when
...
const url = process.env.GODSPEED_RATE_LIMIT_TEST_DATABASE_URL;
if (url) {
  const target = new URL(url);
  if (!['localhost', '127.0.0.1', '[::1]'].includes(target.hostname) || target.pathname !== '/godspeed_rate_limit_test') {
    throw Error('Local godspeed_rate_limit_test required');
  }
}

const clients = [];
async function connect() {
  const c = new Client(url ? { connectionString: url } : { host: '/var/run/postgresql', database: 'godspeed_rate_limit_test', user: 'postgres' });
  await c.connect();
  clients.push(c);
  return c;
}
...
try {
  ...
  const migration = await readFile(new URL('../supabase/migrations/20260909120000_atomic_godspeed_api_rate_limit.sql', import.meta.url), 'utf8');
  await db.query(migration);

  const { rows: [key] } = await db.query(
    'insert into godspeed_api_keys(user_id) values (gen_random_uuid()) returning id'
  );
  ...
  const burst = async (fn, windowStart) => {
    await db.query('delete from godspeed_api_usage');
    ...
    const { rows: [stored] } = await db.query(
      'select request_count from godspeed_api_usage where key_id = $1 and window_start = $2', [key.id, windowStart]
    );
  ...
  const atomic = await burst('godspeed_api_bump_usage', WINDOW);
  ...
  for (let i = 1; i <= LIMIT + 2; i++) {
    const { rows: [r] } = await db.query('select allowed, request_count from godspeed_api_bump_usage($1,$2,$3)', [key.id, WINDOW, LIMIT]);
  ...
  const { rows: [fresh] } = await db.query('select allowed, request_count from godspeed_api_bump_usage($1,$2,$3)', [key.id, next, LIMIT]);
  assert.equal(fresh.request_count, 1, 'a new window starts at one');
  assert.equal(fresh.allowed, true);
  const { rows: [old] } = await db.query('select request_count from godspeed_api_usage where key_id=$1 and window_start=$2', [key.id, WINDOW]);
  assert.equal(old.request_count, LIMIT + 2, 'the previous window must be untouched');

  // 5. Two keys are throttled independently.
  const { rows: [other] } = await db.query('insert into godspeed_api_keys(user_id) values (gen_random_uuid()) returning id');
  const { rows: [independent] } = await db.query('select allowed, request_count from godspeed_api_bump_usage($1,$2,$3)', [other.id, WINDOW, LIMIT]);
  ...
  for (const bad of [0, -1, null]) {
    await assert.rejects(
      () => db.query('select * from godspeed_api_bump_usage($1,$2,$3)', [key.id, WINDOW, bad]),
      /positive integer/,
  ...
  for (const role of ['anon', 'authenticated']) {
    ...
    await assert.rejects(
      () => c.query('select * from godspeed_api_bump_usage($1,$2,$3)', [key.id, WINDOW, LIMIT]),
      /permission denied/i,
      `${role} must not be able to execute godspeed_api_bump_usage`
    );
  }

  console.log('godspeed rate limit: atomic under concurrency, correct sequentially, service-role only');
} finally {
