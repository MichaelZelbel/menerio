...
The browser calls `apply_contact_topic_command(request UUID, command)` using the signed-in owner. MCP calls `apply_contact_topic_command_for_user(owner UUID, request UUID, command)` through its service client, after contacts scope and AI visibility checks. Both enter the same database implementation. A request UUID identifies one intent; transport retries keep its exact payload. Version conflicts require rereading before a new command.
...
1. Verify the feature commit, migration inventory, database backup, and existing function/frontend versions. Apply only `supabase/migrations/20260907140000_contact_topics.sql` to project `tjeapelvjlmbxafsmjef` in a transaction. Record the migration in the project's migration history. Do not bulk-push unrelated pending migrations.
