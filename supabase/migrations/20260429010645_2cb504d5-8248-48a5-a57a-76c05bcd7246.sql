...
CREATE INDEX IF NOT EXISTS idx_action_items_group_membership
ON public.action_items ((metadata->>'group_membership_id'));

CREATE INDEX IF NOT EXISTS idx_action_items_metadata_group_id
ON public.action_items ((metadata->>'group_id'));

ALTER TABLE public.contact_group_memberships
RENAME COLUMN person_id TO contact_id;

ALTER TABLE public.contact_group_memberships
RENAME CONSTRAINT contact_group_memberships_group_person_key TO contact_group_memberships_group_contact_key;

ALTER INDEX IF EXISTS public.idx_contact_group_memberships_person
RENAME TO idx_contact_group_memberships_contact;
...
BEGIN
  ...
  JOIN public.contacts c ON c.id = m.contact_id AND c.user_id = m.user_id
...
AFTER UPDATE OF contact_id, status, position, archived_at ON public.contact_group_memberships
FOR EACH ROW
WHEN (
