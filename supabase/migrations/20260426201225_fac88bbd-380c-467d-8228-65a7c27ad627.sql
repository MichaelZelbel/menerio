  ...
  LOOP
    ...
    IF v_action->>'op' = 'create' THEN
      ...
      ON CONFLICT (user_id, slug) DO UPDATE SET
        title = COALESCE(NULLIF(EXCLUDED.title, ''), public.wiki_pages.title),
        page_type = COALESCE(NULLIF(EXCLUDED.page_type, ''), public.wiki_pages.page_type),
        summary = COALESCE(EXCLUDED.summary, public.wiki_pages.summary),
      ...
      VALUES (
        ...
        v_new_content,
        p_note_id,
        COALESCE(NULLIF(v_action->>'change_summary', ''), 'Created from note'),
        'applied'
      );
    ...
    ELSIF v_action->>'op' = 'update' THEN
      SELECT id, content INTO v_page_id, v_previous_content
      FROM public.wiki_pages
      WHERE user_id = v_user_id AND slug = v_page_slug;

      IF v_page_id IS NULL THEN
        v_new_content := COALESCE(v_action->>'patch', v_action->>'content', '');
        INSERT INTO public.wiki_pages (user_id, slug, title, page_type, summary, content)
        VALUES (
          v_user_id,
          v_page_slug,
        ...
        RETURNING id INTO v_page_id;
      ELSE
        v_new_content := COALESCE(v_action->>'patch', v_previous_content, '');
        INSERT INTO public.wiki_revisions (
          user_id, wiki_page_id, page_slug, page_title, change_type,
          previous_content, new_content, source_note_id, change_summary, status
        )
        VALUES (
          ...
          v_page_id,
          v_page_slug,
          COALESCE(NULLIF(v_action->>'title', ''), (SELECT title FROM public.wiki_pages WHERE id = v_page_id)),
          'updated',
          v_previous_content,
          v_new_content,
          p_note_id,
          COALESCE(NULLIF(v_action->>'change_summary', ''), 'Updated from note'),
          'applied'
        );
