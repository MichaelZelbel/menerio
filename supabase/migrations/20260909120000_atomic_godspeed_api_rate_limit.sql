-- Make Mission Control API rate limit hold under concurrency.
--
-- `_shared/mc-rate-limit.ts` read the window's counter, decided, then wrote the
-- absolute value back:
--
...
-- UNIQUE (key_id, window_start) constraint (godspeed_api_keys_scope_backup migration
...
CREATE OR REPLACE FUNCTION public.godspeed_api_bump_usage(
  p_key_id uuid,
...
BEGIN
  ...
  INSERT INTO public.godspeed_api_usage AS u (key_id, window_start, request_count)
...
COMMENT ON FUNCTION public.godspeed_api_bump_usage(uuid, timestamptz, integer) IS
  'Atomically count one Mission Control API request in its fixed window and say whether it is within the limit. Replaces a read-then-write check that any parallel burst walked straight through.';
...
-- key''s counter.
REVOKE ALL ON FUNCTION public.godspeed_api_bump_usage(uuid, timestamptz, integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.godspeed_api_bump_usage(uuid, timestamptz, integer) FROM anon;
REVOKE ALL ON FUNCTION public.godspeed_api_bump_usage(uuid, timestamptz, integer) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.godspeed_api_bump_usage(uuid, timestamptz, integer) TO service_role;
