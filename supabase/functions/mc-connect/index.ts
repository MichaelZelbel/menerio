...
import { corsHeaders, handleOptions, parsePath } from "../_shared/mc-helpers.ts";
import {
  DEFAULT_APP_URL,
  GODSPEED_CONNECT_ERRORS,
  GODSPEED_CONNECT_LIMITS,
  GODSPEED_GRANT_SCOPES,
  type GodspeedConnectErrorCode,
  type RequestStatus,
  callerAddress,
  ...
  tokenErrorForStatus,
  verificationUris,
} from "../_shared/mc-connect-protocol.ts";

/**
 * Connect a mission control to Menerio without anyone typing a key.
 *
 *   POST /start       Mission Control asks                     no authorization
 *   GET  /request     the approval page reads it       the person's session
 *   POST /approve     the person says yes or no        the person's session
 *   POST /token       Mission Control collects its key, once   the verifier is the proof
 *   GET  /status      a device reports in              Bearer mnr_...
 *   POST /disconnect  end the connection               Bearer mnr_... or the session
 *
 * What is allowed when is decided in _shared/mc-connect-protocol.ts and, for
 * anything that changes more than one row, inside the godspeed_connect_* SQL
...
// The new tables are not in any generated types, and lookupGodspeedKey takes the
...
function fail(code: GodspeedConnectErrorCode, message?: string, extra?: Record<string, string>): Response {
  const known = GODSPEED_CONNECT_ERRORS[code];
...
async function sessionUser(req: Request): Promise<{ id: string; email: string | null } | null> {
  ...
  // A mission control key is not a session; asking the auth server about it would only log noise.
...
/**
 * How the account is named to the person and to the godspeed: its email address,
...
function logDbError(route: string, error: { code?: string; message?: string }) {
  console.error(`[mc-connect] ${route}: ${[error.code, error.message].filter(Boolean).join(" ")}`);
}
...
Deno.serve(async (req) => {
  ...
  // parts[0] = "mc-connect", parts[1] = route
  ...
  try {
    ...
    if (route === "start") {
      ...
      const { data, error } = await admin.rpc("godspeed_connect_start", {
        p_godspeed_id: request.godspeedId,
        p_godspeed_name: request.godspeedName,
        ...
        p_ttl_seconds: GODSPEED_CONNECT_LIMITS.requestTtlSeconds,
        p_max_starts_per_hour: GODSPEED_CONNECT_LIMITS.startsPerHour,
      });
      if (error) {
      ...
      const appUrl = Deno.env.get("GODSPEED_CONNECT_APP_URL") || DEFAULT_APP_URL;
      return json({
        request_id: data.request_id,
        user_code: data.user_code,
        ...verificationUris(appUrl, data.request_id, data.user_code),
        expires_in: GODSPEED_CONNECT_LIMITS.requestTtlSeconds,
        interval: GODSPEED_CONNECT_LIMITS.pollIntervalSeconds,
      });
    }
    ...
    if (route === "request") {
      ...
      const { data, error } = await admin.rpc("godspeed_connect_view", {
    ...
    if (route === "approve") {
      ...
      const { data, error } = await admin.rpc("godspeed_connect_decide", {
        ...
        p_max_wrong_codes: GODSPEED_CONNECT_LIMITS.maxWrongCodes,
      });
      if (error) {
      ...
      switch (data?.outcome) {
        ...
        case "wrong_code": return json({
          error: "wrong_code",
          message: GODSPEED_CONNECT_ERRORS.wrong_code.message,
          attempts_left: data.attempts_left,
        }, GODSPEED_CONNECT_ERRORS.wrong_code.status);
    ...
    if (route === "token") {
      ...
      const minted = await mintGodspeedKey();

      const { data, error } = await admin.rpc("godspeed_connect_collect", {
        p_request_id: body.request_id,
        ...
        p_scopes: [...GODSPEED_GRANT_SCOPES],
        p_min_poll_gap_ms: GODSPEED_CONNECT_LIMITS.minPollGapMs,
        p_max_wrong_verifiers: GODSPEED_CONNECT_LIMITS.maxWrongVerifiers,
      });
      if (error) {
      ...
      switch (data?.outcome) {
        case "slow_down":
          return fail("slow_down", undefined, { "Retry-After": String(GODSPEED_CONNECT_LIMITS.pollIntervalSeconds) });
        case "invalid_grant":
    ...
    if (route === "status") {
      if (req.method !== "GET") return fail("method_not_allowed");
      const key = await godspeedKey(req, admin);
      ...
      // as it always did; Mission Control is only told it is not a connection.
      ...
      const { data, error } = await admin.rpc("godspeed_connect_touch", {
        p_key_id: auth.keyId,
      ...
      return json({
        ...
        connection_id: data.connection_id,
        godspeed_id: data.godspeed_id,
        godspeed_name: data.godspeed_name,
        generation: data.generation,
        account_label: await accountLabel(admin, data.user_id),
        scopes: auth.scopes,
        documents: data.documents,
    ...
    if (route === "disconnect") {
      ...
      // From the godspeed: the key names the connection, nothing in the body does.
      if (authHeader.startsWith("Bearer mnr_")) {
        ...
        const { data, error } = await admin.rpc("godspeed_connect_disconnect", {
          p_user_id: null,
    ...
    console.error(`[mc-connect] ${route || "?"}: ${(err as Error).message}`);
    return fail("server_error");
  }
});

type GodspeedKeyCheck =
  | { auth: { userId: string; scopes: string[]; keyId: string; connectionId: string | null }; error: null; code: null }
  | { auth: null; error: Response; code: GodspeedConnectErrorCode };

/**
 * The `Bearer mnr_...` key of a request, through the same lookup every Mission Control API
 * function uses, with the refusal worded the way this function words errors.
 */
async function godspeedKey(req: Request, admin: AdminClient): Promise<GodspeedKeyCheck> {
  const authHeader = req.headers.get("Authorization") ?? "";
  if (!authHeader.startsWith("Bearer mnr_")) {
    return { auth: null, error: fail("invalid_key", "Send Mission Control's key as 'Authorization: Bearer mnr_...'."), code: "invalid_key" };
  }
  const { result, errorCode, errorMessage } = await lookupGodspeedKey(authHeader.slice("Bearer ".length), admin);
  if (result) return { auth: result, error: null, code: null };
  const code: GodspeedConnectErrorCode =
...
async function belongsToEndedConnection(admin: AdminClient, apiKey: string): Promise<boolean> {
  const { data: keyRow } = await admin
    ...
    .select("user_id, godspeed_connection_id")
    .eq("key_hash", await sha256Hex(apiKey.trim()))
    .maybeSingle();
  if (!keyRow?.godspeed_connection_id) return false;
  const { data: connection } = await admin
    .from("godspeed_connections")
    .select("status")
    .eq("id", keyRow.godspeed_connection_id)
