...
async function generateInAppNotifications(userId: string): Promise<void> {
  ...
  // Weekly reviews are generated by the dedicated weekly-review function.
}

async function triggerWeeklyReviewSchedule(): Promise<void> {
  const now = new Date();
  if (now.getDay() !== 5) return;

  const response = await fetch(`${SUPABASE_URL}/functions/v1/weekly-review`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ mode: "scheduled", days: 7 }),
  });

  if (!response.ok) {
    const body = await response.text();
    console.error(`weekly-review schedule trigger failed [${response.status}]: ${body}`);
  }
}
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    EdgeRuntime.waitUntil(triggerWeeklyReviewSchedule());
