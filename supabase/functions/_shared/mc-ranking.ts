/**
 * One policy for how a mirrored mission control file ranks against a note the user wrote.
 *
 * Mission Control mirror is thousands of machine-maintained Markdown files sitting in
 ...
 *   - `mc-api-notes` GET /search
 *   - the app's keyword scorer, through the twin in `src/lib/mc-ranking.ts`
...
/** A mission control file's similarity is multiplied by this before anything is ordered. */
export const GODSPEED_SIMILARITY_FACTOR = 0.85;

/** Where ranking is tiered, a mission control file lands this many tiers below its match. */
export const GODSPEED_TIER_DEMOTION = 1;

/** The folder tree the mirror owns. Nothing else may be filed in or under it. */
export const GODSPEED_FOLDER_ROOT = "godspeed";

export type NoteSourceFilter = "all" | "native" | "godspeed";
...
export function rankingSimilarity(
  ...
  return isGodspeedMirror(sourceApp) ? similarity * GODSPEED_SIMILARITY_FACTOR : similarity;
}
...
export function demoteTier(tier: number, sourceApp?: string | null): number {
  return isGodspeedMirror(sourceApp) ? tier + GODSPEED_TIER_DEMOTION : tier;
}
...
export function matchesSourceFilter(
  ...
  if (filter === "native") return !isGodspeedMirror(sourceApp);
  if (filter === "godspeed") return isGodspeedMirror(sourceApp);
  return true;
}
 ...
 * tree would show "Godspeed/x" and "godspeed/x" as two trees that look like one.
 */
export function isGodspeedFolderPath(path: string | null | undefined): boolean {
  const p = (path ?? "").trim().toLowerCase();
  return p === GODSPEED_FOLDER_ROOT || p.startsWith(`${GODSPEED_FOLDER_ROOT}/`);
}
 ...
 */
export function godspeedFileLabel(
  sourceApp?: string | null,
  sourceId?: string | null,
): string | null {
  if (!isGodspeedMirror(sourceApp)) return null;
  const id = (sourceId ?? "").trim();
  return id ? `[godspeed file: ${id}]` : "[godspeed file]";
}
...
/**
 * Deterministic order for merged semantic + text rows: tier (godspeed demoted),
 * discounted similarity, native before godspeed on a tie, recency, then the order
 ...
 * tier; with native-first ahead of similarity a mission control file could only ever show
