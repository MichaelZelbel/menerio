/**
 * The rules of the "connect your hub" flow, with nothing else in the file.
 *
 * A hub (a folder of files on someone's computer that their AI assistants work
 * from) asks for access, the person confirms in the browser, and the hub
 * collects a key. `hub-connect/index.ts` does the talking and the SQL functions
 * in 20260920120000_hub_connections.sql do the writing; every number, every
 ...
 * An approved request can still expire: the hub has the same ten minutes to
...
export function isPolledTooSoon(lastPollAt: string | Date | null, now: Date = new Date()): boolean {
  ...
  return now.getTime() - new Date(lastPollAt).getTime() < HUB_CONNECT_LIMITS.minPollGapMs;
}
...
export const HUB_CONNECT_ERRORS = {
  ...
  wrong_code: { status: 400, message: "That code does not match. Compare it with the one your hub shows." },
  unauthorized: { status: 401, message: "Sign in to Menerio first." },
  invalid_key: { status: 401, message: "This key is not valid." },
  revoked: { status: 401, message: "This hub's connection to Menerio was ended." },
  access_denied: { status: 403, message: "The request was declined." },
  legacy_key: { status: 403, message: "This key was not made by connecting a hub, so there is no connection to end. Revoke it under Settings, API Keys." },
  ...
  expired_token: { status: 410, message: "This request has expired or its key was already collected. Start again from the hub." },
...
export type HubConnectErrorCode = keyof typeof HUB_CONNECT_ERRORS;
 ...
 */
export function tokenErrorForStatus(status: RequestStatus): HubConnectErrorCode | null {
  switch (status) {
...
/** `claude-code`, `codex`, `hub-bot`: a short lowercase name, nothing a page could not show. */
...
/**
 * A hub or device name fit to show on the approval page: control characters
...
/**
 * What the hub asked for, reduced to the two switches that exist. Context is
...
export function parseStartRequest(body: Record<string, unknown>): { request: StartRequest; problem: null } | { request: null; problem: string } {
  ...
  if (!isUuid(body.hub_id)) return fail("hub_id must be a UUID");
  if (!isUuid(body.device_id)) return fail("device_id must be a UUID");
  const hubName = cleanDisplayName(body.hub_name);
  if (!hubName) return fail("hub_name is required");
...
/**
 * The X-Hub-* headers of GET /status. Anything malformed is dropped rather than
...
export function parseContactHeaders(get: (name: string) => string | null): ContactHeaders {
  const deviceId = get("x-hub-device-id");
  const client = (get("x-hub-client") ?? "").trim().toLowerCase();
  const state = (get("x-hub-client-state") ?? "").trim().toLowerCase();
  const validClient = CLIENT_NAME_PATTERN.test(client) ? client : null;
  return {
    deviceId: isUuid(deviceId) ? deviceId.toLowerCase() : null,
    deviceName: cleanDisplayName(get("x-hub-device-name")),
    client: validClient,
...
export async function hashCallerAddress(address: string, secret: string): Promise<string> {
  ...
  const signature = await crypto.subtle.sign("HMAC", key, encoder.encode(`hub-connect:${address}`));
