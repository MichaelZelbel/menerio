...
export async function checkRateLimit(
  ...
  //
  // `godspeed_api_bump_usage` does the increment relative to the stored value in a
  ...
  // `scripts/test-godspeed-rate-limit.mjs` runs both versions against real concurrent
  // connections so the difference stays proven.
  const { data, error } = await supabaseAdmin.rpc("godspeed_api_bump_usage", {
    p_key_id: keyId,
  ...
  if (error) {
    ...
    // take the whole Mission Control API offline for legitimate clients. The counter is the
    // thing that broke, so the honest response is to serve the request and make
    // the fault loud rather than silent.
    console.error(
      `[mc-rate-limit] usage counter unavailable for key=${keyId}: ` +
