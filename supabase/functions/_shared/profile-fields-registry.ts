...
/**
 * DB-backed near-duplicate label resolution.
 *
 * Returns the label that should actually be used for a write when the incoming
 * label is a near-duplicate of a registry canonical label, one of its aliases,
 * or a label already stored for this subject in the same category. Returns
 * null when the label is genuinely new.
 *
 * The same logic runs inside the `profile_entry_canonicalize` trigger, so this
 * is a read-only preview used to keep duplicates out of the review queue.
 */
export async function resolveProfileLabel(
  ...
  if (!args.categoryId || !args.label) return null;
  try {
    const { data, error } = await db.rpc("profile_resolve_label", {
      _user_id: args.userId,
      _contact_id: args.contactId,
      _category_id: args.categoryId,
      _category_slug: args.categorySlug,
      _label: args.label,
    });
    if (error) {
      console.error("[profile-fields-registry] resolveProfileLabel failed", error);
      return null;
    }
    const resolved = typeof data === "string" ? data.trim() : "";
    return resolved.length > 0 ? resolved : null;
  } catch (e) {
    console.error("[profile-fields-registry] resolveProfileLabel threw", e);
    return null;
  }
}
