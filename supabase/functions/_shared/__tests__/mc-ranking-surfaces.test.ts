...
/**
 * Mission Control ranking policy is one file; these tests hold every surface that ranks
 ...
 * is covered by mc-ranking.test.ts, which tests the function it calls.
 */
...
describe("search-notes-semantic", () => {
  it("orders by discounted similarity: a mission control file needs a clear lead to pass a native note", async () => {
    const notes = [note("mc-close", "godspeed"), note("native", "web"), note("mc-far-ahead", " GODSPEED ")];
    const body = await semanticSearch({ notes, chunks: [
      { note_id: "mc-close", similarity: 0.80, content: "c", note_title: "mc-close" },      // 0.68 to order by
      { note_id: "native", similarity: 0.70, content: "c", note_title: "native" },
      { note_id: "mc-far-ahead", similarity: 0.90, content: "c", note_title: "mc-far-ahead" }, // 0.765
    ] });
    expect(body.mode).toBe("semantic");
    expect(body.results.map((r) => r.id)).toEqual(["mc-far-ahead", "native", "mc-close"]);
  ...
  it("puts native notes first in the no-credits text fallback, recency kept inside each group", async () => {
    const notes = [note("mc-new", "godspeed", "2026-09-01T00:00:00Z"), note("native-old", "web", "2026-01-01T00:00:00Z"), note("native-new", null, "2026-08-01T00:00:00Z")];
    const body = await semanticSearch({ tokens: 0, notes, chunks: [] });
    expect(body.mode).toBe("ilike_fallback_no_credits");
    expect(body.results.map((r) => r.id)).toEqual(["native-old", "native-new", "mc-new"]);
  });
});
...
describe("every other ranking surface is wired to the one policy", () => {
  ...
  it("MCP results label a mission control file, and search_notes takes a source", () => {
    const mcp = read("supabase/functions/menerio-mcp/index.ts");
    const fmt = mcp.slice(mcp.indexOf("function formatNoteResult("), mcp.indexOf("function noteText("));
    expect(fmt).toContain("godspeedFileLabel(t.source_app, t.source_id)");
    expect(fmt.indexOf("godspeedFileLabel")).toBeLessThan(fmt.indexOf('if (view === "metadata")'));
    expect(mcp).toContain('source: z.enum(["all", "native", "godspeed"]).optional().default("all")');
    expect(mcp).toContain("hybridSearchNotes(query, limit, threshold, source)");
  });

  it("Mission Control API search ranks through the same function", () => {
    expect(read("supabase/functions/_shared/note-search.ts")).toContain("rankHybridRows(Array.from(byNote.values()), query)");
    expect(read("supabase/functions/mc-api-notes/index.ts")).toContain("combinedNoteSearch(supabase,");
  });

  it("the app's keyword scorer uses the frontend twin, and its callers select source_app", () => {
    expect(read("src/lib/search-terms.ts")).toContain('from "@/lib/mc-ranking"');
