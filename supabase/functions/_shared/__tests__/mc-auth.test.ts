...
/**
 * lookupGodspeedKey is the one function in front of the MCP server and every
 * mc-api-* function. Two things have to hold at once: a key that belongs to no
 * godspeed connection is read exactly as it always was, and a key that does belong
 * to one works only while that connection is active and of the key's generation.
 *
 * The real mc-auth.ts is bundled (its supabase import stubbed out, since the
...
let lookupGodspeedKey: Lookup;

beforeAll(async () => {
  const bundle = await build({
    entryPoints: ["supabase/functions/_shared/mc-auth.ts"], bundle: true, write: false, platform: "node", format: "esm",
    plugins: [{
  ...
  ({ lookupGodspeedKey } = await import(/* @vite-ignore */ `data:text/javascript;base64,${source}`));
});
...
async function database(overrides: { connection?: Row; grant?: Row; legacy?: Row } = {}) {
  return memoryDb({
    godspeed_api_keys: [
      { id: "key-legacy", user_id: "user-a", key_hash: await sha256Hex(LEGACY), scopes: ["notes"], is_active: true, expires_at: null, godspeed_connection_id: null, generation: null, ...overrides.legacy },
      { id: "key-grant", user_id: "user-a", key_hash: await sha256Hex(GRANT), scopes: ["notes", "profile"], is_active: true, expires_at: null, godspeed_connection_id: "conn-a", generation: 2, ...overrides.grant },
      { id: "key-old", user_id: "user-a", key_hash: await sha256Hex(OLD_GRANT), scopes: ["notes"], is_active: true, expires_at: null, godspeed_connection_id: "conn-a", generation: 1 },
    ],
...
describe("a key that belongs to no connection", () => {
  it("is accepted exactly as before, without a second query", async () => {
    ...
    const { result, errorMessage, errorCode } = await lookupGodspeedKey(LEGACY, client);
    expect(result).toEqual({ userId: "user-a", scopes: ["notes"], keyId: "key-legacy", connectionId: null });
    expect(errorMessage).toBeNull();
    expect(errorCode).toBeNull();
    expect(reads).toEqual({ godspeed_api_keys: 1 });
  });

  it("keeps its old refusals, word for word", async () => {
    expect((await lookupGodspeedKey("not-a-key", (await database()))).errorMessage)
      .toBe("Missing or invalid API key. Expected 'Bearer mnr_...' header.");
    expect((await lookupGodspeedKey("mnr_" + "f".repeat(48), await database())).errorMessage).toBe("Invalid API key.");
    expect((await lookupGodspeedKey(LEGACY, await database({ legacy: { is_active: false } }))).errorMessage)
      .toBe("API key has been revoked.");
    expect((await lookupGodspeedKey(LEGACY, await database({ legacy: { expires_at: "2020-01-01T00:00:00Z" } }))).errorMessage)
      .toBe("API key has expired.");
  });

  it("is untouched by whatever happens to a mission control connection of the same account", async () => {
    const db = await database({ connection: { status: "revoked" } });
    expect((await lookupGodspeedKey(LEGACY, db)).result?.keyId).toBe("key-legacy");
  });
  ...
  it("still works on a database that does not have the connection columns yet", async () => {
    ...
    const client = {
      from(table: string) {
        ...
        q.select = (columns: string) => {
          asked++;
          if (columns.includes("godspeed_connection_id")) {
            const refusal = { data: null, error: { code: "42703", message: "column godspeed_api_keys.godspeed_connection_id does not exist" } };
    ...
    const { result } = await lookupGodspeedKey(LEGACY, client);
...
describe("a key minted for a mission control connection", () => {
  it("is accepted while the connection is active and of its generation, at the cost of one query", async () => {
    const { client, reads } = counting(await database());
    const { result } = await lookupGodspeedKey(GRANT, client);
    expect(result).toEqual({ userId: "user-a", scopes: ["notes", "profile"], keyId: "key-grant", connectionId: "conn-a" });
    expect(reads).toEqual({ godspeed_api_keys: 1, godspeed_connections: 1 });
  });

  it("is refused once Mission Control was connected again, even if it was never switched off", async () => {
    const { result, errorMessage, errorCode } = await lookupGodspeedKey(OLD_GRANT, await database());
    expect(result).toBeNull();
    expect(errorMessage).toBe("This mission control's connection to Menerio was ended.");
    expect(errorCode).toBe("connection_ended");
  });

  it("is refused once the connection was ended", async () => {
    const lookup = await lookupGodspeedKey(GRANT, await database({ connection: { status: "revoked" } }));
    expect(lookup.result).toBeNull();
    expect(lookup.errorMessage).toBe("This mission control's connection to Menerio was ended.");
  });

  it("names the connection, not the key, when the key itself was switched off", async () => {
    const lookup = await lookupGodspeedKey(GRANT, await database({ grant: { is_active: false } }));
    expect(lookup.errorMessage).toBe("This mission control's connection to Menerio was ended.");
    expect(lookup.errorCode).toBe("connection_ended");
  });

  it("cannot borrow another account's connection", async () => {
    const lookup = await lookupGodspeedKey(GRANT, await database({ grant: { godspeed_connection_id: "conn-b" } }));
    expect(lookup.result).toBeNull();
    expect(lookup.errorCode).toBe("connection_ended");
  });

  it("is refused when the connection is gone altogether", async () => {
    const lookup = await lookupGodspeedKey(GRANT, await database({ grant: { godspeed_connection_id: "conn-missing" } }));
    expect(lookup.result).toBeNull();
  });
  ...
  it("says 'try again', not 'ended', when the connection could not be read", async () => {
    ...
    const client = {
      from(table: string) {
        if (table !== "godspeed_connections") return db.from(table);
        const refusal = { data: null, error: { code: "57014", message: "canceling statement due to statement timeout" } };
        const dead: Row = { select: () => dead, eq: () => dead, maybeSingle: () => dead, then: (ok: (v: unknown) => unknown) => Promise.resolve(ok(refusal)) };
        return dead;
      },
    };
    const lookup = await lookupGodspeedKey(GRANT, client);
    expect(lookup.result).toBeNull();
    expect(lookup.errorCode).toBe("unavailable");
