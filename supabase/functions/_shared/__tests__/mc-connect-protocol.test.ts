...
import {
  ...
  verificationUris,
  verifyS256,
} from "../mc-connect-protocol.ts";
import { mintGodspeedKey } from "../mc-key-mint.ts";
...
describe("the request state machine", () => {
  ...
  it("names the same statuses as the table's CHECK constraint", () => {
    const sql = readFileSync("supabase/migrations/20260920120000_godspeed_connections.sql", "utf8");
...
describe("limits", () => {
  it("are the numbers of the contract", () => {
    expect(GODSPEED_CONNECT_LIMITS.maxWrongCodes).toBe(5);
    expect(GODSPEED_CONNECT_LIMITS.maxWrongVerifiers).toBe(5);
    expect(GODSPEED_CONNECT_LIMITS.requestTtlSeconds).toBe(600);
    expect(GODSPEED_CONNECT_LIMITS.startsPerHour).toBe(10);
    expect(GODSPEED_CONNECT_LIMITS.pollIntervalSeconds).toBe(3);
    // Enforced a little below what is advertised, never above it.
    expect(GODSPEED_CONNECT_LIMITS.minPollGapMs).toBeLessThanOrEqual(GODSPEED_CONNECT_LIMITS.pollIntervalSeconds * 1000);
    expect(GODSPEED_CONNECT_LIMITS.minPollGapMs).toBeGreaterThan(0);
  });
  ...
  it("grants what a hand-made key for a mission control carries today", () => {
    expect([...GODSPEED_GRANT_SCOPES]).toEqual(
...
describe("error codes", () => {
  it("map to the HTTP statuses of the contract", () => {
    const statuses = Object.fromEntries(Object.entries(GODSPEED_CONNECT_ERRORS).map(([code, e]) => [code, e.status]));
  ...
  it("carry a plain message each, with none of the words the house style bans", () => {
    for (const { message } of Object.values(GODSPEED_CONNECT_ERRORS)) {
...
describe("what Mission Control sends", () => {
  const valid = {
    godspeed_id: "0B4F0B52-5A0B-4A7B-9C0E-3D7C1E2F4A55",
    godspeed_name: "  Michael's\u0000 mission control \n",
  ...
  it("accepts a well-formed /start and tidies it", () => {
    ...
    expect(request).toEqual({
      godspeedId: "0b4f0b52-5a0b-4a7b-9c0e-3d7c1e2f4a55",
      godspeedName: "Michael's mission control",
  ...
  it("names the first thing wrong with a /start", () => {
    const broken: Array<[Record<string, unknown>, RegExp]> = [
      [{ ...valid, godspeed_id: "nope" }, /godspeed_id/],
  ...
  it("reads the X-Godspeed-* headers and drops what is malformed", () => {
    const headers = (h: Record<string, string>) => (name: string) => h[name] ?? null;
    expect(parseContactHeaders(headers({
      "x-godspeed-device-id": "7D3C1D0E-64A0-4C58-8F2A-1B9E6F0C2D11",
      "x-godspeed-device-name": " Laptop ",
      "x-godspeed-client": "Claude-Code",
      "x-godspeed-client-state": "working",
    }))).toEqual({
      deviceId: "7d3c1d0e-64a0-4c58-8f2a-1b9e6f0c2d11",
      deviceName: "Laptop",
      client: "claude-code",
    ...
    expect(parseContactHeaders(headers({
      "x-godspeed-device-id": "laptop",
      "x-godspeed-client": "<script>",
      "x-godspeed-client-state": "working",
    }))).toEqual({ deviceId: null, deviceName: null, client: null, clientState: null });
    expect(parseContactHeaders(headers({ "x-godspeed-client": "codex", "x-godspeed-client-state": "thriving" })).clientState).toBeNull();
...
describe("the links /start returns", () => {
  it("carry the request and the code, and only those", () => {
    ...
    expect(uris.verification_uri).toBe("https://menerio.com/connect-godspeed");
    expect(uris.verification_uri_complete).toBe(
      "https://menerio.com/connect-godspeed?request=7d3c1d0e-64a0-4c58-8f2a-1b9e6f0c2d11&code=BCDF-GHJK",
    );
  });
});
...
describe("minting a key", () => {
  it("makes mnr_ plus 48 hex characters, a 12 character prefix, and the SHA-256 that is stored", async () => {
    ...
    expect((await mintGodspeedKey()).fullKey).not.toBe(minted.fullKey);
  });
});
